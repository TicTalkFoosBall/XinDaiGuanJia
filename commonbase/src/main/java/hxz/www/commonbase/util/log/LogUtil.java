package hxz.www.commonbase.util.log;
...
import com.xingxiu.frame.api.Api;
import com.xingxiu.frame.api.BaseObserver;
import com.xingxiu.frame.app.BaseApplication;
import com.xingxiu.frame.cache.SpCache;
import com.xingxiu.frame.model.result.BaseResult;
import com.xingxiu.frame.model.result.QNTokenModel;
import com.xingxiu.frame.rx.RxHelper;
import com.xingxiu.frame.rx.RxUtil;
import com.xingxiu.frame.rx.task.RxIOTask;
import com.xingxiu.frame.util.DeviceUtil;
import com.xingxiu.frame.util.file.FileUtil;
import com.xingxiu.frame.util.time.DateUtil;
...
public class LogUtil extends LogShow {
    ...
    private static String getFunctionName() {
        ...
        for (StackTraceElement st : sts) {
            ...
            if (st.getClassName() != null && st.getClassName().contains("LogUtil"))
                continue;
    ...
    private static void writeFileSdcard(final String message) {
        ...
        logExecutor.execute(() -> {
            try {
                ...
                FileOutputStream fout = new FileOutputStream(mLogFile.getPath() + File.separator + getLogFileName(), true);
                fout.write((message + "\r\n").getBytes("UTF-8"));
                fout.flush();
                fout.close();
            } catch (Throwable e) {
    ...
    public static String getLogFileName() {
        ...
        return mUserId + "_android_" + verionName + "_" + DateUtil.format(new Date(), "yyyy-MM-dd") + ".log";
    }
        ...
        try {
            ...
            if (!fileDir.exists()) {
                ...
                FileFilter filter = pathname -> {
                    String date = isForce ? "yyyy-MM-dd" : DateUtil.format(new Date(), "yyyy-MM-dd");
