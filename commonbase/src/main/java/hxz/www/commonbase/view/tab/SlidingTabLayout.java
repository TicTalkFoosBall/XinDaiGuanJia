package hxz.www.commonbase.view.tab;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private boolean mTextSizeScaleWhenScroll;//滑动时 tab textview 文字大小及颜色变化
    private boolean mScaleWhenScroll = false;//当mTextSizeScaleWhenScroll=true   点击tab间隔>1时 不设置textview 变化
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorGradientColors = getIndicatorGradientColors(ta, R.styleable.SlidingTabLayout_tl_indicator_gradient_colors);
        ...
        mTextSizeScaleWhenScroll = ta.getBoolean(R.styleable.SlidingTabLayout_tl_textSizeScaleWhenScroll, false);
        ...
        mScaleWhenScroll = mTextSizeScaleWhenScroll;
        ta.recycle();
    }
     ...
     * @return
     */
    private int[] getIndicatorGradientColors(TypedArray ta, @StyleableRes int styleableRes) {
        //resId==0表示直接使用#cccccc这种模式，不为0说明可以获取到资源类型
        int resId = ta.getResourceId(styleableRes, 0);
        if (resId != 0) {
            String typeName = mContext.getResources().getResourceTypeName(resId);
            if (!TextUtils.isEmpty(typeName) && typeName.equals("array")) {
                ...
                int[] bgColorArray = new int[length];//多个颜色
                String strIndex;
                int intIndex;
            ...
            public void onClick(View v) {
                ...
                if (position != -1) {
                    if (mViewPager.getCurrentItem() != position) {
                        if (mTextSizeScaleWhenScroll) {
                            if (Math.abs(mViewPager.getCurrentItem() - position) > 1) {
                                mScaleWhenScroll = false;
                                isNeedupdateTabSelection = true;
                            } else {
                                mScaleWhenScroll = true;
                            }
                        }
                        if (mSnapOnTabClick) {
    ...
    private boolean isNeedupdateTabSelection = false; //当mTextSizeScaleWhenScroll =true时 文字选中状态在scrollToCurrentTab()方法中设置

    @Override
    public void onPageSelected(int position) {
        mCurrentSelectPosition = position;
        if (isNeedupdateTabSelection) {
            updateTabSelection(position);
        }
        if (mTextSizeScaleWhenScroll) {
            isNeedupdateTabSelection = false;
        }
    }
    ...
    private void scrollToCurrentTab() {
        ...
        if (mTextSizeScaleWhenScroll && mScaleWhenScroll) {
            float scale = 0;
            if (mCurrentTab != mCurrentSelectPosition) {//向左滑动
                if (mCurrentPositionOffset == 0) {
                    scale = 1;
                } else {
                    if (mCurrentTab > mCurrentSelectPosition) {
                        scale = mCurrentPositionOffset;
                    } else {
                        scale = 1 - mCurrentPositionOffset;
                    }
                }
                float offTextSize = (mTextSelectSize - mTextsize) * scale;
                updateTabTitleSize(mCurrentSelectPosition, mTextSelectSize - offTextSize, getColor(scale), scale < 0.7);
                updateTabTitleSize(mCurrentTab, mTextsize + offTextSize, getColor(1 - scale), scale > 0.3);
            } else {//向右滑动
                if (mCurrentPositionOffset > 0 && mCurrentSelectPosition < mTabCount - 1) {
                    scale = mCurrentPositionOffset;
                    float offTextSize = (mTextSelectSize - mTextsize) * scale;
                    updateTabTitleSize(mCurrentSelectPosition, mTextSelectSize - offTextSize, getColor(scale), scale < 0.7);
                    updateTabTitleSize(mCurrentTab + 1, mTextsize + offTextSize, getColor(1 - scale), scale > 0.3);
                } else {
                    updateTabSelection(mCurrentSelectPosition);
                }
            }
        }
    ...
    private void updateTabTitleSize(int position, float textSize, final int color, boolean bold) {
        ...
        if (tab_title != null) {
            ...
            if (mTextBold == TEXT_BOLD_BOTH) {
                ...
                tab_title.getPaint().setFakeBoldText(bold);
            }
        }
    }
    ...
    public int getColor(float radio) {
        int redStart = Color.red(mTextSelectColor);
        int blueStart = Color.blue(mTextSelectColor);
        int greenStart = Color.green(mTextSelectColor);
        int redEnd = Color.red(mTextUnselectColor);
        int blueEnd = Color.blue(mTextUnselectColor);
        int greenEnd = Color.green(mTextUnselectColor);
