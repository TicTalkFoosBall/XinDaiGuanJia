package yokeyword.fragmentation;
...
import yokeyword.fragmentation.exception.AfterSaveStateTransactionWarning;
...
import yokeyword.fragmentation.queue.Action;
import yokeyword.fragmentation.queue.ActionQueue;
...
class TransactionDelegate {
    ...
    void loadMultipleRootTransaction(final FragmentManager fm, final int containerId, final int showPosition, final String[] tags, final ISupportFragment... tos) {
        enqueue(fm, new Action(Action.ACTION_LOAD) {
            @Override
            public void run() {
                FragmentTransaction ft = fm.beginTransaction();
